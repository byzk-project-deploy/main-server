...
const (
	testName    = "BYPT LOCAL SERVER 127.0.0.1"
	testDnsName = "unix"
)
...
func TestGeneratorPemCerts(t *testing.T) {
	...
	certs, err := Instance.GeneratorServerPemCerts(testName, testDnsName, testIp)
	if !a.NoError(err) {
		return
	}
	...
	if certLen == 1 {
		...
		clientCert, err := Instance.GeneratorClientPemCert(testName, testDnsName, testIp)
		if !a.NoError(err) {
			return
		}
